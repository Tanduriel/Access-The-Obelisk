...
case $os_type in
    Linux*)
        executable_path="$BASEDIR/AcrossTheObelisk.x86_64"
        lib_postfix="so"
        ;;
    Darwin*)
        executable_name=$(basename "Across the Obelisk" .app)
        real_executable_name=$(defaults read "$BASEDIR/Across the Obelisk.app/Contents/Info" CFBundleExecutable)
        executable_path="$BASEDIR/Across the Obelisk.app/Contents/MacOS/${real_executable_name}"
        lib_postfix="dylib"
        ;;
    *)
